...
RUN apk --update add \
  ...
  curl -s https://api.github.com/repos/containous/traefik/releases/latest | jq .assets[].browser_download_url | grep -E linux-amd64 | xargs wget -qO- -O /usr/local/bin/traefik && \
